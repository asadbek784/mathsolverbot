...
GEMINI_KEY  = "YOUR_GEMINI_KEY_HERE"  # aistudio.google.com dan (TEKINGA)
...
def ask_gemini(prompt, system="Siz matematik va umumiy bilim bo'yicha yordamchisiz. O'zbek tilida qisqa va aniq javob bering."):
    ...
    if not key or key == "YOUR_GEMINI_KEY_HERE":
        return None, (
            "❌ Gemini API kalit sozlanmagan!\n\n"
            "Admin sifatida:\n"
            "/setkey AIzaSy... yuboring\n\n"
            "Kalit olish: aistudio.google.com → Get API Key (BEPUL)"
        )
    try:
        contents = [
            {"role": "user", "parts": [{"text": "System: " + system}]},
            {"role": "model", "parts": [{"text": "Tushundim."}]},
        ...
        if not r.ok:
            return None, "❌ Gemini xatosi: " + str(r.status_code)
        data = r.json()
...
def kb_main(admin=False):
    ...
    rows = [
        [{"text": "Ilovani ochish", "web_app": {"url": url}}],
    ...
    try:
        if "message" in update:
            ...
            if text == "/start":
                ...
                # Admin Gemini kalitni o'rnatishi
                parts = text.split(" ", 1)
                if len(parts) == 2 and len(parts[1]) > 10:
                    db_set("ms_ai_key", parts[1].strip())
                    send(cid, "✅ Gemini API kalit saqlandi!")
                else:
